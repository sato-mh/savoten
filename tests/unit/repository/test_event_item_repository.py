...
class TestSave:
    ...
    @pytest.mark.parametrize('event_item',[EventItem('test_name', [candidate])])
    def test_succeeds_if_event_item_has_no_id(self, event_item):
        event_item_repository.save(event_item)
        assert get_public_vars(event_item_repository.event_items[1]) == get_public_vars(event_item)

    @pytest.mark.parametrize('event_item',[EventItem('test_name', [candidate],id = 3)])
    def test_update_succeeds_if_event_item_has_id(self, event_item):
        event_item_repository.save(event_item)
        event_item.name = 'updated_name'
        event_item_repository.save(event_item)
        assert get_public_vars(event_item_repository.event_items[3]) == get_public_vars(event_item)


class TestDelete:

    @pytest.mark.parametrize('event_item',[EventItem('test_name', [candidate],id = 1)])
...
class TestFindById:
    ...
    def test_return_event_item_if_id_exists(self, event_item):
        ...
        assert get_public_vars(found_event_item) == get_public_vars(event_item_repository.event_items[1])
...
class TestFindByEventId:
    ...
    def test_return_found_event_items_if_event_id_exists(self,add_event_items_to_repository):
